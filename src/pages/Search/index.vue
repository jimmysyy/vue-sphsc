                ...
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a
                    >综合<span
                      v-show="isOne"
                      class="iconfont"
                      :class="{ 'icon-down': isDesc, 'icon-top': isAsc }"
                    ></span
                  ></a>
                </li>
...
export default {
  ...
  data() {
    return {
      ...
      searchParams: {
        ...
        props: [], //商品属性的搜索条件
        trademark: "", //品牌的搜索条件
        order: "1:desc", //排序的参数 【默认初始值:1:desc(综合)】
  ...
  beforeMount() {
    //es6的解构语法,要复习一下
    //在发请求之前，把接口需要传递参数，进行整理（在给服务器发请求之前，把参数整理好，服务器就回返回查询的数据）
